...
import time
import urllib.parse
import json # Necessário para ledger e hashing
from datetime import datetime # Necessário para ledger
import hashlib # Necessário para identidade e ledger

# Criptografia para chaves e assinaturas (do módulo cryptography)
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.asymmetric import ec
from cryptography.hazmat.primitives import serialization

# REMOVIDOS IMPORTS DO QISKIT para simulação clássica de QKD
# from qiskit import QuantumCircuit
# from qiskit.providers.aer import AerSimulator # Importação corrigida para AerSimulator

##############################################
# MÓDULOS CRIPTOGRÁFICOS BÁSICOS (Wallet, Identity, Ledger)
# Integrados a partir dos arquivos fornecidos pelo usuário.
##############################################
...
def montar_transacao(participant_fp, tipo, dados_extra):
    """
    Monta uma estrutura de transação padrão para o ledger.
    Inclui o fingerprint do participante, tipo da transação, timestamp e dados adicionais.
    """
    transacao = {
        'type': tipo,
        'participant': participant_fp,
        'timestamp': datetime.now().isoformat(),
        **dados_extra # Descompacta dados_extra no dicionário da transação
    }
    return transacao

def gerar_hash_transacao(transacao):
    """
    Gera o hash SHA256 de uma transação.
    A transação é convertida para uma string JSON (com chaves ordenadas para consistência)
    e depois codificada para bytes antes do hashing.
    """
    # A ordenação das chaves é crucial para garantir que o hash seja o mesmo para a mesma transação
    return hashlib.sha256(json.dumps(transacao, sort_keys=True).encode()).hexdigest()

def assinar_transacao(private_key, transacao):
    """
    Assina uma transação usando a chave privada fornecida.
    A transação é convertida para uma string JSON e assinada usando ECDSA com SHA256.
...
def verificar_assinatura(public_key_pem, transacao, assinatura_hex):
    """
    Verifica a validade de uma assinatura de transação usando a chave pública.
    public_key_pem: Chave pública em formato PEM (string).
    transacao: O objeto da transação original que foi assinado.
    ...
    try:
        # Tenta verificar a assinatura. Se a verificação falhar, uma exceção é levantada.
        public_key.verify(
            bytes.fromhex(assinatura_hex), # Converte a assinatura hex para bytes
            json.dumps(transacao, sort_keys=True).encode(), # Converte a transação para bytes para verificação
            ec.ECDSA(hashes.SHA256())
        )
        return True
    except Exception as e:
        # Captura qualquer exceção (por exemplo, InvalidSignature)
        # print(f"Falha na verificação de assinatura: {e}") # Descomente para depuração
        return False

class Participant:
    """
    Representa um participante no sistema de incentivo, com sua própria carteira (chaves)
    e uma impressão digital única.
    """
...
class QuantumIncentiveEngine:
    ...
    Gerencia participantes e registra transações (recompensas).
    Este é o coração da "criptomoeda" simplificada para PoR.
    """
    def __init__(self):
        self.participants = {} # Dicionário de participantes registrados
        self.ledger = []       # Lista de transações (ledger)
    ...
    def registrar_participante(self, nome):
        ...
        if nome not in self.participants:
            ...
            return True
        print(f"[Crypto Engine] Participante '{nome}' já registrado.")
        return False

    def recompensa(self, nome_participante, valor, metadata=None):
        """
        Cria e registra uma transação de recompensa para um participante.
        Assina a transação com a chave privada do participante.
        valor: O valor da recompensa.
        metadata: Dicionário opcional de dados adicionais para a transação (ex: hash da pesquisa).
        """
        if nome_participante not in self.participants:
            print(f"[Crypto Engine] ERRO: Participante '{nome_participante}' não encontrado para recompensa.")
            return None

        p = self.participants[nome_participante]
        # Monta a transação com tipo 'reward' e o valor
        tx_data = {'valor': valor}
        if metadata:
        ...
        tx['hash'] = gerar_hash_transacao(tx) # Gera o hash da transação
        
        self.ledger.append(tx) # Adiciona a transação ao ledger
        print(f"[Crypto Engine] ✅ Transação de recompensa assinada por '{nome_participante}': {tx['hash'][:10]}... (Valor: {valor})")
        return tx

    def verificar_ultima_transacao(self, nome_participante):
        """
        Verifica a assinatura da última transação registrada no ledger,
        para um determinado participante.
        """
        if not self.ledger:
            print("[Crypto Engine] Ledger vazio. Nenhuma transação para verificar.")
            return False
        
        if nome_participante not in self.participants:
            print(f"[Crypto Engine] ERRO: Participante '{nome_participante}' não encontrado para verificação.")
            return False

        tx = self.ledger[-1] # Pega a última transação
        pub_key = self.participants[nome_participante].public_key
        
        # Verifica se a transação realmente pertence ao participante (pelo fingerprint)
        if tx.get('participant') != self.participants[nome_participante].fingerprint:
            print(f"[Crypto Engine] 🧪 Última transação não pertence a '{nome_participante}'.")
            return False

        # Cria uma cópia da transação sem a assinatura para verificar
        ...
        print(f"[Crypto Engine] 🧪 Assinatura da última transação para '{nome_participante}' válida: {valido}")
        return valido
...
class SyntheticMindSystem:
    def __init__(self):
        ...
        incluindo o motor de incentivo criptográfico.
        """
        ...
        # ATUALIZADO: Usando o simulador QKD clássico
        self.qkd_simulator = ClassicalQKD_E91_Simulator() 
        
        # Inicializa o motor de incentivo criptográfico
        self.incentive_engine = QuantumIncentiveEngine()
        # Registra o próprio sistema AeonCosma como um participante para receber recompensas
    ...
    def run_qkd_experiment(self, num_pairs=100):
        """
        Executa a simulação CLÁSSICA do protocolo QKD E91 e retorna a correlação
        da chave como métrica de desempenho.
        """
        print(f"Iniciando experimento QKD E91 CLÁSSICO com {num_pairs} pares...")
        correlation = self.qkd_simulator.run_experiment(num_pairs=num_pairs)
        print(f"Experimento QKD E91 CLÁSSICO concluído. Correlação: {correlation:.2f}")
        return correlation
    ...
    def issue_research_reward(self, participant_name):
        ...
        na última métrica combinada de pesquisa gerada pela IA.
        A "valor" da recompensa pode ser escalado pela métrica.
        """
        if participant_name not in self.incentive_engine.participants:
            print(f"ERRO: Participante '{participant_name}' não registrado para receber recompensa.")
            return

        # Define o valor da recompensa. Exemplo: 1 token por unidade da métrica combinada.
        # Ajuste a escala conforme necessário para o seu "tokenomics".
        reward_value = self.last_combined_metric # A própria métrica como valor
        
        # Opcional: Adicionar metadados da pesquisa (e.g., hash do DNA, métrica)
        metadata = {
        ...
        print(f"\nTentando emitir recompensa de pesquisa para '{participant_name}' com valor baseado na métrica: {reward_value:.4f}")
    ...
    def export_all_information(self):
        ...
        Inclui também o estado do ledger criptográfico.
        """
        info = {
            "cosmology": {
                "H0": self.cosmo.H0,
...
##############################################
# MÓDULO 10: DISTRIBUIÇÃO DE CHAVE QUÂNTICA - ClassicalQKD_E91_Simulator
# ATUALIZADO para simulação CLÁSSICA.
##############################################

class ClassicalQKD_E91_Simulator:
    def __init__(self):
        """
        Inicializa o simulador CLÁSSICO do protocolo QKD E91.
        Não usa bibliotecas de computação quântica como Qiskit.
        """
        pass # Não há backend quântico para inicializar aqui.

    def _simulate_measurement(self, prepared_bit, basis):
        """
        Simula a medição de um bit (0 ou 1) em uma base 'Z' ou 'X'.
        Em uma simulação clássica, a 'medição' na base 'X' é como
        introduzir uma chance de erro ou uma escolha aleatória,
        imitando a natureza probabilística da mecânica quântica
        quando as bases não coincidem.
        
        - Se a base for 'Z', o bit é "medido" como foi preparado.
        - Se a base for 'X', há uma chance de 50% de que o bit "medido"
          seja o inverso do bit preparado, simulando a superposição.
          (Isso é uma simplificação para fins de simulação clássica do efeito)
        """
        if basis == 'Z':
            return prepared_bit
        elif basis == 'X':
            # Simula a probabilidade de 50% de obter um resultado diferente
            # quando a base é "incompatível" (como na base X para um bit Z)
            return random.randint(0, 1) # Retorna 0 ou 1 aleatoriamente
        return None # Caso de base inválida

    def run_experiment(self, num_pairs=100):
        """
        Executa a simulação CLÁSSICA completa do protocolo QKD E91 para um determinado
        número de pares.
        
        Protocolo Simplificado:
        1. Alice gera um bit aleatório e escolhe uma base aleatória ('Z' ou 'X').
        2. Bob escolhe uma base aleatória ('Z' ou 'X').
        3. Alice "envia" o bit para Bob (simulado).
        4. Bob "mede" o bit na sua base escolhida.
        5. Alice e Bob comparam publicamente suas BASES.
        6. Se as BASES coincidem, eles comparam os RESULTADOS.
           A correlação é calculada com base nos resultados onde as bases coincidiram.
        """
        alice_bases = [random.choice(['Z', 'X']) for _ in range(num_pairs)]
        bob_bases = [random.choice(['Z', 'X']) for _ in range(num_pairs)]
        
        # Bits preparados por Alice (que seriam os qubits emaranhados no QKD real)
        # Para simulação clássica, vamos gerar bits aleatórios que ambos conhecem
        # conceitualmente antes da "medição" simulada.
        prepared_bits = [random.randint(0, 1) for _ in range(num_pairs)]
        ...
        for i in range(num_pairs):
            ...
            # Alice "mede" seu lado do par emaranhado (simulado)
            # Na simulação clássica, isso é o bit original na base dela.
            # No E91 real, Alice mede seu qubit para obter um resultado.
            # Aqui, para simular a natureza do E91, assumimos que se a base é Z,
            # o resultado da medição é o bit original. Se a base é X, pode ser 0 ou 1.
            # Para manter a correlação para bases iguais, simplificamos:
            # Se bases iguais, resultado_alice = prepared_bit (para o propósito de comparação de chave)
            # Se bases diferentes, resultado_alice é aleatório (não entra na chave)
            
            # Para o E91, a chave vem da CORRELAÇÃO em bases iguais.
            # Simulação do resultado para Alice e Bob, que teoricamente deveriam ser correlacionados.
            # Simplificação: ambos "preparam" um bit. Se bases coincidem, esperamos que seja o mesmo.
            # Se não coincidem, o "ruído quântico" é simulado pela aleatoriedade de Bob.

            # Resultado da "medição" de Bob na sua base
            # Se a base de Bob é Z, ele mede o bit preparado.
            # Se a base de Bob é X, ele mede aleatoriamente.
            # A "correlação" vem da natureza dos pares emaranhados:
            # quando medidos na mesma base, seus resultados deveriam ser os mesmos.
            
            # Neste simulador clássico, para o E91, o "bit preparado" é o que ambos "teriam"
            # Se as bases são as mesmas, Alice e Bob "concordam" no resultado.
            # Se as bases são diferentes, o resultado de Bob é "aleatório" em relação ao de Alice.
            
            result_alice = prepared_bits[i] # Alice sempre "tem" o bit original
            result_bob = self._simulate_measurement(prepared_bits[i], b_basis) # Bob mede com sua base

            if a_basis == b_basis: # Alice e Bob comparam bases publicamente
                total_checked_pairs += 1
                if result_alice == result_bob: # Se bases iguais, resultados devem ser iguais (idealmente)
                    total_matches += 1
...
def run_dsl_script(script, system):
    ...
    Permite controlar a execução dos módulos da IA, incluindo as funcionalidades criptográficas.
    ...
    - QKD_EXPERIMENT [num_pairs]: Executa a simulação CLÁSSICA de Distribuição de Chave Quântica.
    - REGISTER_PARTICIPANT <nome>: Registra um novo participante no motor criptográfico.
    - ISSUE_RESEARCH_REWARD <nome_participante>: Emite uma recompensa para o participante com base na última métrica de pesquisa.
    - VERIFY_LAST_TX <nome_participante>: Verifica a assinatura da última transação para um participante.
    """
...
if __name__ == '__main__':
    ...
    dsl_script = """
    # --- FASE 1: Geração de Pesquisa e Evolução da IA ---
    # Executa simulações iniciais para que a IA gere dados e métricas
    COSMO_SIMULATE
    EXO_SIMULATE
    DBPEDIA_EXPERIMENT Cosmologia
    QKD_EXPERIMENT 50 # Executa a simulação CLÁSSICA QKD com 50 pares
    
    # Evolve o DNA Cósmico, usando a métrica combinada como feedback.
    # Esta etapa representa a "descoberta" ou "otimização" da IA.
    EVOLVE_DNA
    ...
    # --- FASE 2: Geração e Verificação de Recompensa (PoR Simplificado) ---
    # Registra o "AeonCosma" como o participante que receberá a recompensa
    # (já feito na inicialização do SyntheticMindSystem, mas pode ser feito explicitamente aqui)
    # REGISTER_PARTICIPANT AeonCosma 
    
    # Emite uma recompensa para o participante "AeonCosma" baseada na métrica combinada.
    # Isto simula a "mineração" ou cunhagem de tokens por "Prova de Pesquisa".
    ISSUE_RESEARCH_REWARD AeonCosma
    
    # Verifica a validade da última transação emitida
    ...
    QKD_EXPERIMENT 100 # Executa a simulação CLÁSSICA QKD com mais pares
    EVOLVE_DNA # A IA evolui novamente com o novo feedback
    
    # Emite outra recompensa baseada na nova métrica de pesquisa
    ISSUE_RESEARCH_REWARD AeonCosma
    VERIFY_LAST_TX AeonCosma

    # --- FASE 4: Exportação de Informações Finais ---
