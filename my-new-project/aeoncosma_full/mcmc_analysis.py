...
try:
    # Usando sep=r'\s+' para lidar com múltiplos espaços como delimitador (o 'r' remove o SyntaxWarning)
    # e 'comment='#' caso haja linhas de comentário começando com '#'
    sn_df = pd.read_csv(file_path_sn, sep=r'\s+', comment='#')
    
    # Renomeando as colunas para os nomes esperados pelo script
    # 'zHD' para 'z'
    # 'MU_SH0ES' para 'mu'
    # 'MU_SH0ES_ERR_DIAG' para 'sigma_mu'
    ...
    # Verificando se as colunas necessárias existem após o renomeamento
...
# Funções Omega_ond e Omega_EDE (componentes adicionais do seu modelo)
...
# Modelo Híbrido (Exemplo: wCDM + EDE + Omega_ond)
...
# Integrando para obter a distância de luminosidade
...
# Log-verossimilhança para Supernovas
...
ndim = 6 # Número de parâmetros
nwalkers = 50 # Número de "caminhantes" na cadeia MCMC
nsteps = 2000 # Número de passos da cadeia

# Ponto inicial para os caminhantes (ajuste conforme a sua expectativa)
p0_initial = np.array([70.0, 0.3, 0.01, -3.0, 0.05, 3000.0])

# Espalhando os caminhantes em torno do ponto inicial
...
# Plot das cadeias (para verificar convergência)
